...
Most bugs tell you they exist — a crash, a wrong number, a failing test. Security bugs stay silent until someone exploits them, so the discipline has to be built into how you write code, not caught after. Agents introduce them by trusting input from outside, moving an authority check to where it can be bypassed, leaking a secret into a commit, or pulling in an unvetted dependency. These skills target the most common and most damaging: **who is authoritative for a decision**, **where untrusted input gets validated**, **where secrets are allowed to live**, and **what code you let into the tree**.
...
- Adding a new third-party dependency, action, or base image → **vet-new-dependencies**.
...
| [`vet-new-dependencies`](skills/vet-new-dependencies/SKILL.md) | Adding a new dependency | Check name, maintenance, license, and necessity before it enters the tree |
