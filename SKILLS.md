# Skills, Sources & Agent Helpers

This is the browseable inventory for the repository. External implementations stay in their upstream repositories; original reusable packages live under `bespoke/`.

## Bespoke first-party packages

- [`delivery-loop`](bespoke/delivery-loop/README.md) — single-session implementation workflow: triage → Plan → optional Architect → Build → Code Review → Verify. Includes install guidance plus first-party `plan-gate` and `scope-fence` companions.
- [`catalog-review`](bespoke/catalog-review/README.md) — independent review agent for requirement fit, provenance/licensing, research integrity, readability, installability and open-source readiness.

## Individual external skills

### Workflow & hygiene

- [`systematic-debugging`](catalog/workflow-hygiene/systematic-debugging.md) — root-cause-first debugging from `obra/superpowers`.
- [`verification-before-completion`](catalog/workflow-hygiene/verification-before-completion.md) — fresh evidence before completion claims from `obra/superpowers`.

### UI / UX

- [`frontend-design`](catalog/ui-ux/frontend-design.md) — distinctive production-grade interface design from Anthropic.
- [`ui-ux-pro-max`](catalog/ui-ux/ui-ux-pro-max.md) — design-system, palette, typography, UX, motion and chart guidance.
- [`web-design-guidelines`](catalog/ui-ux/web-design-guidelines.md) — Vercel interface/accessibility review guidance.

### Frontend engineering

- [`vercel-composition-patterns`](catalog/frontend-engineering/vercel-composition-patterns.md) — scalable React composition patterns.
- [`vercel-react-best-practices`](catalog/frontend-engineering/vercel-react-best-practices.md) — React/Next.js performance guidance.

### Three.js

- [`threejs-fundamentals`](catalog/threejs/threejs-fundamentals.md)
- [`threejs-geometry`](catalog/threejs/threejs-geometry.md)
- [`threejs-materials`](catalog/threejs/threejs-materials.md)
- [`threejs-lighting`](catalog/threejs/threejs-lighting.md)
- [`threejs-textures`](catalog/threejs/threejs-textures.md)
- [`threejs-animation`](catalog/threejs/threejs-animation.md)
- [`threejs-loaders`](catalog/threejs/threejs-loaders.md)
- [`threejs-shaders`](catalog/threejs/threejs-shaders.md)
- [`threejs-postprocessing`](catalog/threejs/threejs-postprocessing.md)
- [`threejs-interaction`](catalog/threejs/threejs-interaction.md)

## Prominent upstream repositories

The dated selection method and grouping live in [`catalog/sources/README.md`](catalog/sources/README.md). This inventory links the cards without duplicating mutable popularity metrics.

### Engineering / general-purpose

- [`obra/superpowers`](catalog/sources/general-engineering/obra-superpowers.md)
- [`mattpocock/skills`](catalog/sources/general-engineering/mattpocock-skills.md)
- [`anthropics/skills`](catalog/sources/general-engineering/anthropics-skills.md)
- [`addyosmani/agent-skills`](catalog/sources/general-engineering/addyosmani-agent-skills.md)
- [`openai/skills`](catalog/sources/general-engineering/openai-skills.md)
- [`vercel-labs/agent-skills`](catalog/sources/general-engineering/vercel-agent-skills.md)
- [`google/skills`](catalog/sources/general-engineering/google-skills.md)
- [`huggingface/skills`](catalog/sources/general-engineering/huggingface-skills.md)
- [`github/awesome-copilot`](catalog/sources/general-engineering/github-awesome-copilot.md)
- [`alirezarezvani/claude-skills`](catalog/sources/general-engineering/alirezarezvani-claude-skills.md)
- [`Jeffallan/claude-skills`](catalog/sources/general-engineering/jeffallan-claude-skills.md)
- [`muratcankoylan/Agent-Skills-for-Context-Engineering`](catalog/sources/general-engineering/context-engineering-skills.md)

### Specialist / behavioural

- [`multica-ai/andrej-karpathy-skills`](catalog/sources/specialist/andrej-karpathy-skills.md)
- [`nextlevelbuilder/ui-ux-pro-max-skill`](catalog/sources/specialist/ui-ux-pro-max-skill.md)
- [`kepano/obsidian-skills`](catalog/sources/specialist/obsidian-skills.md)
- [`K-Dense-AI/scientific-agent-skills`](catalog/sources/specialist/scientific-agent-skills.md)
- [`JuliusBrussee/caveman`](catalog/sources/specialist/caveman.md)

### Standards / discovery / harnesses

- [`agentskills/agentskills`](catalog/sources/standards-and-discovery/agentskills-standard.md)
- [`sickn33/agentic-awesome-skills`](catalog/sources/standards-and-discovery/agentic-awesome-skills.md)
- [`ComposioHQ/awesome-claude-skills`](catalog/sources/standards-and-discovery/awesome-claude-skills.md)
- [`affaan-m/ECC`](catalog/sources/standards-and-discovery/ecc.md)

## Maintainer docs

- [`CONTRIBUTING.md`](CONTRIBUTING.md) — how to add/refresh catalogue and bespoke material.
- [`catalog/README.md`](catalog/README.md) — external-card conventions.
- [`bespoke/catalog-review/checklist.md`](bespoke/catalog-review/checklist.md) — pre-publication review gate.
